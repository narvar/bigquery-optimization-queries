...
(
  ...
  FROM
    ...
    -- Change the narvar-data-lake and region-us placeholders below to match your required values
    `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.RESERVATIONS_TIMELINE,
...
(
    ...
    FROM
      `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.JOBS_TIMELINE_BY_ORGANIZATION
    WHERE
      period_start BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL interval_in_days DAY)
        AND CURRENT_TIMESTAMP()
