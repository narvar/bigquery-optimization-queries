-- This query will show the aggregate average slot usage of all query jobs for each second in the specified timeframe.
...
BEGIN
    WITH src AS (
      SELECT
        ...
        period_start
      FROM
        `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.JOBS_TIMELINE
      WHERE
