...
DECLARE location_query_template STRING DEFAULT "SET @@location = 'region-us';\n";
DECLARE physical_storage_query_template STRING DEFAULT "ALTER SCHEMA `doitintl-cmp-bq` SET OPTIONS(storage_billing_model = 'PHYSICAL')";
DECLARE logical_storage_query_template STRING DEFAULT "ALTER SCHEMA `doitintl-cmp-bq` SET OPTIONS(storage_billing_model = 'LOGICAL')";
...
(
  ...
  FROM
    ...
    `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.TABLE_STORAGE AS tb
  ...
  -- End user: Change to reflect your project and region
  JOIN `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.TABLES AS t
...
(
  ...
  */
  ...
  FROM
    -- End user: Change to reflect your project and region
    `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.SCHEMATA_OPTIONS
  WHERE
    ...
    IF(
      ...
      IF(
        ...
        CONCAT(
          ...
          REPLACE(physical_storage_query_template, 'doitintl-cmp-bq', dataset))
      ),
      -- Is on physical storage currently
      IF(
        logical_storage_price < physical_storage_price,
        CONCAT(
          ...
          REPLACE(logical_storage_query_template, 'doitintl-cmp-bq', dataset))
        ,
         -- Do nothing
         NULL
        )
