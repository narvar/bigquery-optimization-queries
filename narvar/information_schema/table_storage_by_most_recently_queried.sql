 ...
 *  1. Modify the narvar-data-lake and region-us placeholders below to match your required values. 
...
WITH tables AS (
    ...
    FROM
        `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.JOBS_BY_PROJECT AS j,
...
table_billing_type AS (
    ...
    FROM
        `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.TABLE_STORAGE AS ts
    INNER JOIN
        -- Join with TABLES view to get the original creation time
        `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.TABLES AS t
    ...
    LEFT JOIN
        ...
        `narvar-data-lake`.`region-us`.INFORMATION_SCHEMA.TABLE_OPTIONS AS topt
