-- DTPL-6903: Reservation Utilization Analysis During Queue Delays
-- 
-- Purpose: Check if bq-narvar-admin-US.default reservation was saturated
--          when messaging queries experienced long wait times
--
-- Strategy:
-- 1. Identify time periods when messaging queries had high queue times
-- 2. Check total slot utilization across ALL users in that reservation
-- 3. Identify who/what was consuming slots during those periods
--
-- Expected bytes processed: ~100-150 GB (7 days audit logs for entire reservation)
...
-- Step 1: Find periods when messaging queries had delays
