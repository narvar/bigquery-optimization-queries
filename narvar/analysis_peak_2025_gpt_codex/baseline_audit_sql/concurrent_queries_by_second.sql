-- Concurrent query jobs by second with parentJobName filtering to avoid counting child script steps.
...
WITH raw_events AS (
  SELECT
    ...
    TIMESTAMP_TRUNC(protopayload_auditlog.servicedata_v1_bigquery.job.jobStatistics.startTime, SECOND) AS startTime,
    TIMESTAMP_TRUNC(protopayload_auditlog.servicedata_v1_bigquery.job.jobStatistics.endTime, SECOND) AS endTime,
    TIMESTAMP_DIFF(
